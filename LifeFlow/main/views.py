...
# Local
from .forms import TaskForm, HealthMetricForm, HealthProfileForm, ReminderForm
from .models import (
    Bill, Document, Task,
    Family, FamilyMembership, FamilyInvite,
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect(next_url or 'appboard:home')
...
def health_manager(request):
    ...
    if creds and _has_required_scopes(creds, required_scopes):
        try:
...
def ai_query(request):
    try:
        ...
        if not client:
            return JsonResponse({"ok": False, "error": "OpenAI not configured."}, status=503)
        ...
        logging.exception("OpenAI quota exceeded / rate-limited")
        return JsonResponse({"ok": False, "error": "OpenAI quota exceeded or rate-limited."}, status=429)
    except AuthenticationError:
        logging.exception("OpenAI authentication failed")
        return JsonResponse({"ok": False, "error": "OpenAI authentication failed."}, status=401)
    except APIError:
        logging.exception("OpenAI API error")
        return JsonResponse({"ok": False, "error": "OpenAI API error."}, status=502)
    except OpenAIError:
...
def google_connect(request):
    ...
    flow = Flow.from_client_config(_google_flow_config(redirect_uri), scopes=requested_scopes, state=state)
    flow.redirect_uri = redirect_uri

    have_refresh = bool(request.session.get(GOOGLE_SESSION_KEY, {}).get("refresh_token"))
    auth_kwargs = dict(access_type="offline", include_granted_scopes="true", state=state)
    if not have_refresh:
...
def google_callback(request):
    ...
    redirect_uri = settings.GOOGLE_REDIRECT_URI
    requested_scopes = request.session.pop("google_requested_scopes", GOOGLE_ID_SCOPES + GCAL_SCOPES)

    flow = Flow.from_client_config(_google_flow_config(redirect_uri), scopes=None, state=expected_state)
    flow.redirect_uri = redirect_uri
    flow.fetch_token(authorization_response=request.build_absolute_uri(), state=expected_state)
    credentials = flow.credentials

    # identify user via ID token
    idinfo = id_token.verify_oauth2_token(credentials.id_token, GoogleRequest(), settings.GOOGLE_CLIENT_ID)
...
def google_fit_connect(request):
    flow = Flow.from_client_config(_google_flow_config(settings.GOOGLE_FIT_REDIRECT_URI), scopes=GFIT_SCOPES)
...
def google_fit_callback(request):
    ...
    flow = Flow.from_client_config(_google_flow_config(settings.GOOGLE_FIT_REDIRECT_URI), scopes=GFIT_SCOPES, state=expected_state)
    flow.redirect_uri = settings.GOOGLE_FIT_REDIRECT_URI
    flow.fetch_token(authorization_response=request.build_absolute_uri())
    credentials = flow.credentials
